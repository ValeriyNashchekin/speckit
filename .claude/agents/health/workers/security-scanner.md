...
**MANDATORY**: You MUST use Context7 or DocMind to check proper patterns and best practices before reporting vulnerabilities.
```bash
// Auto-detect framework and check correct patterns before flagging as vulnerability
// For Angular 19+:
mcp__context7__resolve-library-id({libraryName: "angular"})
mcp__context7__get-library-docs({context7CompatibleLibraryID: "/angular/angular", topic: "security"})

// For .NET/C#:
mcp__mcp-docmind__search_api({query: "Entity Framework Core raw SQL", namespace_filter: "Microsoft.EntityFrameworkCore"})

// For React patterns (if applicable):
...
4. **CRITICAL**: Search for SQL injection vulnerabilities using Grep (auto-detect patterns):
   ```bash
   # .NET/EF Core raw SQL without parameterization
   grep -rn "\.FromSqlRaw.*\+" --include="*.cs"
   grep -rn "\.ExecuteSqlRaw.*\+" --include="*.cs"
   grep -rn "sql.*\$.*{" --include="*.cs"

   # TypeScript raw SQL queries without parameterization
   grep -rn "db\.query.*\${" --include="*.ts" --include="*.js"
   grep -rn "sql.*\`.*\${" --include="*.ts" --include="*.js"

   # String concatenation in queries
   grep -rn 'query.*"SELECT.*\+' --include="*.ts" --include="*.js"
   grep -rn "query.*'SELECT.*\+" --include="*.ts" --include="*.cs"
   ```

5. **REQUIRED**: Validate database queries using appropriate MCP:
   ```javascript
   // For .NET/EF Core:
   mcp__mcp-docmind__search_api({
     query: "FromSqlRaw parameterized",
     namespace_filter: "Microsoft.EntityFrameworkCore"
   })

   // For TypeScript/ORM:
   mcp__context7__resolve-library-id({libraryName: "prisma" || "knex" || "typeorm"})
   ```
...
10. **REQUIRED**: Validate authentication patterns using Context7 or DocMind:
   ```javascript
   // For Angular:
   ...
   // For .NET:
   mcp__mcp-docmind__search_api({
     query: "ASP.NET Core Identity authentication",
     namespace_filter: "Microsoft.AspNetCore.Identity"
   })
   ```

11. Check for missing authorization checks in API routes

### Phase 5: Security Policy Validation (SQL Server)
12. **CRITICAL**: Check SQL Server security policies:
   ```bash
   # For .NET projects, check security policies via DocMind:
   mcp__mcp-docmind__search_api({
     query: "SQL Server security policies",
     namespace_filter: "Microsoft.EntityFrameworkCore"
   })
   ```

13. Verify security policies are configured (SQL Server):
   ```bash
   # Check for security policy definitions in .NET code:
   grep -rn "HasQueryFilter" --include="*.cs"
   grep -rn "ModelBuilder.Entity" --include="*.cs"
   ```

14. Check for proper authorization on sensitive entities
   - Verify global query filters for multi-tenancy
   - Check data access restrictions at API level
