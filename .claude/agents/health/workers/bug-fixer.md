...
**MANDATORY**: You MUST use Context7 or DocMind to check correct patterns before implementing any fix.
```javascript
// Auto-detect framework and get best practices before fixing
// For Angular 19+:
...
4. **Single Task Execution Protocol**
   ...
   - Run validation tests INCLUDING PRODUCTION BUILD (auto-detect stack):
     * **For Angular**: `npm run build` (AOT compilation catches type errors)
     * **For .NET**: `dotnet build` (Roslyn validates all code)
     * **For TypeScript**: `tsc --noEmit` AND appropriate build command
