...
```bash
// Auto-detect framework and check correct patterns before flagging as bug
// For Angular 19+:
mcp__context7__resolve-library-id({libraryName: "angular"})
mcp__context7__get-library-docs({context7CompatibleLibraryID: "/angular/angular", topic: "signals"})

// For .NET/C#:
mcp__mcp-docmind__search_api({query: "Entity Framework Core", namespace_filter: "Microsoft.EntityFrameworkCore"})
...
6. Run available linters and type checkers using Bash (auto-detect stack):
   - **For Angular**: `npx ng lint`, `npm run build`
   - **For .NET**: `dotnet build`, `dotnet test`
   - **For TypeScript/JavaScript**: `npx tsc --noEmit`, `npm run lint`
...
7. **CRITICAL: Test Production Build** (for web projects):
   - **Angular**: `npm run build` - AOT compilation catches errors missed by tsc
   - **.NET**: `dotnet build` - Roslyn compiler validates all code
   - Common build-only errors to watch for:
     * Spread operator type errors (Angular)
     * EF Core query type inference failures (.NET)
     * Dependency injection configuration errors (.NET)
     * Missing inject decorators (Angular)
...
14. Check for common code quality problems:
    ...
    - **TypeScript strictness issues**:
      ...
      * EF Core query type inference problems (.NET)
      * Observable type inference issues (Angular)
